...
We are pleased to announce a new and improved show for the spring. New scenes feature procedural animations derived from the historical Blooming Leaf weaving pattern. Unlike ordinary fabrics, our simulated blooming leaves convey motion as their thread counts change with time, and we have promoted the pattern itself from its traditional two dimensions into 3D. When the animated lights illuminate the underlying woven metal, this dynamic intersection of digital and physical weaves generates rhythmic interference patterns in multiple hues.
...
* In one separate thread, a ShowRunner object runs a quasiperiodic event loop, one iteration per **frame**. In each frame, **commands** may be pending, in which case one more pending **commands** are drained from the **command queue** and processed.
